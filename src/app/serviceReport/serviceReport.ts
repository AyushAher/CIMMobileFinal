...
export class ServiceReportComponent implements OnInit {
  ...
  GeneratePDF(preview: boolean) {
    if (!preview) {
      if (!this.isCompleted && confirm(`Do you really want to send the service report to below customer emails:\n \t${this.ServiceRequest.operatoremail} \n\t ${this.ServiceRequest.email}`)) {
        this.pdf(preview);
      }
    } else {
      this.pdf(preview);
  ...
  pdf(preview: boolean) {
    if (preview) {
      ...
      link.href = `https://service.avantgardeinc.com/servicereport/${this.ServiceReportId} ?showPdf=true`
      link.click();
      link.remove();
      return;
    }
