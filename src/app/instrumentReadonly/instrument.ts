...
export class InstrumentRonlyComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.listTypeService.getById("INSTY")
      .pipe(first()).subscribe((data: ListTypeItem[]) => this.instuType = data);

    this.distributorService.getAll()
      .pipe(first()).subscribe((data: any) => this.distibutorList = data.object);

    this.listTypeService.getById(this.code)
      .pipe(first()).subscribe((data: ListTypeItem[]) => this.listTypeItems = data);
